...
function BillRow({ bill, onEdit, onRemove, onSetStatus }) {
  const title = bill.isInstallment
    ? `${bill.installment?.description || bill.name} — Parcela ${bill.installment?.current || 1}/${bill.installment?.total || 1}`
    : bill.name;
        ...
        <div className="bill-title-line">
          ...
          {bill.isInstallment && (
            <span className="pill neutral">
              Parcela {bill.installment?.current || 1}/{bill.installment?.total || 1}
            </span>
          )}
